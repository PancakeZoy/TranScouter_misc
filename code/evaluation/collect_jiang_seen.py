...
output_path = ROOT / "results/collected/jiang/seen_pred_results.pkl"
...
for i in range(10):
    ...
    cond_transfer = test_conds[test_conds.cov_unseen & (~test_conds.pert_unseen)]

    # TranScouter
    pred_trans_i = trans_result[f'split_{i}']['Prediction']
    pred_trans_i = pred_trans_i[pred_trans_i.cov_pert.isin(cond_transfer.index)].groupby('cov_pert').mean()
    pred_trans.append(pred_trans_i)
    
    # Simple average across all conditions of the same perturbation
    avg_i = []
    for cond in cond_transfer.index:
    ...
    # scGen
    pred_scgen_i = sc.read_h5ad(ROOT / f"results/baselines/scgen/jiang/jiang_{i}.h5ad")
    ...
    # trVAE
    pred_trvae_i = sc.read_h5ad(ROOT / f"results/baselines/trvae/jiang/jiang_{i}.h5ad")
    pred_trvae_i = pd.DataFrame(pred_trvae_i.X, index=pred_trvae_i.obs.index,
                                columns=pred_trvae_i.var_names)
    pred_trvae.append(pred_trvae_i)
...
pred_trvae = pd.concat(pred_trvae).loc[pred_trans.index]
...
data = {'TranScouter': pred_trans, 'Avg': pred_avg, 'scGen': pred_scgen, 
        'trVAE': pred_trvae, 'True': true, 'Ctrl': ctrl}
