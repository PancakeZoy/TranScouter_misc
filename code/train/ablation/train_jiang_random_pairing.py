...
ADATA_UNS_PATH = ROOT / "data" / "jiang" / "processed" / "adata_uns.pkl"
OUT_DIR = ROOT / "results" / "ablation" / "random_pairing"
...
# shuffle the control cells
ctrl_row_idx = np.where(adata.obs["perturbation"] == "control")[0]
np.random.seed(24)
ctrl_row_idx_shuffled = np.random.permutation(ctrl_row_idx)
new_order = np.arange(adata.n_obs)
new_order[ctrl_row_idx] = ctrl_row_idx_shuffled
...
with open(ADATA_UNS_PATH, "rb") as f:
    adata_uns = pickle.load(f)
    pertdata.adata.uns = adata_uns
