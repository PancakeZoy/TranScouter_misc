...
OUT_DIR = ROOT / "results" / "baselines" / "scgen" / "mcfaline"
...
def main(split_i):
    ...
    for index, row in tqdm(test_df.iterrows(), total=test_df.shape[0]):
        ...
        pred = sc.get.aggregate(pred, by='cov_pert', func='mean')
        pred.X = pred.layers['mean']
        del pred.layers['mean']
        pred.obs['covariate'] = cov_i
        pred.obs['perturbation'] = pert_i
        pred.obs['cov_pert'] = covpert_i
        pred.obs = pred.obs[['covariate', 'perturbation', 'cov_pert']].copy()
    ...
    pred_split.write_h5ad(OUT_DIR / f"mcfaline_{split_i}.h5ad")
